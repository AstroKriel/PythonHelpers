...
class Tests(unittest.TestCase):
    ...
    def test_cast_to_string(self):
        ...
        ## with conjunction
        result = list_utils.as_string(["a", "b", "c"], conjunction="and")
        self.assertEqual(result, "a, b and c")
        ## with conjunction and quotes
        result = list_utils.as_string(["a", "b", "c"], wrap_in_quotes=True, conjunction="or")
        self.assertEqual(result, "`a`, `b` or `c`")
        ## two elements with conjunction
        result = list_utils.as_string(["a", "b"], conjunction="or")
    ...
    def test_get_intersect_of_lists(self):
        ...
        output = list_utils.get_intersect_of_lists(list_a=[1, 2, 3], list_b=[2, 3, 4])
        expected = [2, 3]
        self.assertEqual(sorted(output), sorted(expected))
        ## no intersection
        output = list_utils.get_intersect_of_lists(list_a=[1, 2, 3], list_b=[4, 5, 6])
        self.assertEqual(output, [])
        ## identical lists
        output = list_utils.get_intersect_of_lists(list_a=[1, 2, 3], list_b=[1, 2, 3])
        self.assertEqual(sorted(output), [1, 2, 3])
        ## one list is empty
        output = list_utils.get_intersect_of_lists(list_a=[], list_b=[1, 2, 3])
        self.assertEqual(output, [])

    def test_get_union_of_lists(self):
        ## union of two lists
        output = list_utils.get_union_of_lists(list_a=[1, 2, 3], list_b=[2, 3, 4])
        self.assertEqual(sorted(output), [1, 2, 3, 4])
        ## no common elements, just merge both lists
        output = list_utils.get_union_of_lists(list_a=[1, 2, 3], list_b=[4, 5, 6])
        self.assertEqual(sorted(output), [1, 2, 3, 4, 5, 6])
        ## identical lists, should return one set of values
        output = list_utils.get_union_of_lists(list_a=[1, 2, 3], list_b=[1, 2, 3])
        self.assertEqual(sorted(output), [1, 2, 3])
        ## one list is empty
        output = list_utils.get_union_of_lists(list_a=[], list_b=[1, 2, 3])
        self.assertEqual(output, [1, 2, 3])

    def test_get_index_of_closest_value(self):
        ## typical case 1: target value exists in the list
        output_index = list_utils.get_index_of_closest_value(values=[1, 5, 8], target=5)
        self.assertEqual(output_index, 1)
        ## typical case 2: target value does not exist, find closest value
        output_index = list_utils.get_index_of_closest_value(values=[1, 5, 8], target=6)
        self.assertEqual(output_index, 1)
    ...
    def test_get_index_of_first_crossing(self):
        ...
        index = list_utils.get_index_of_first_crossing(values=values, target=0.6, direction="rising")
        self.assertEqual(index, 2)  # crosses between 0.5 (index 2) and 0.7 (index 3)
        ## falling crossing
        values = [1.0, 0.9, 0.6, 0.4, 0.2]
        index = list_utils.get_index_of_first_crossing(values=values, target=0.5, direction="falling")
        self.assertEqual(index, 2)  # crosses between 0.6 and 0.4
        ## non-directional (any) crossing
        values = [0.9, 0.7, 0.4, 0.6, 0.8]
        index = list_utils.get_index_of_first_crossing(values=values, target=0.5)
        ...
        with self.assertRaises(ValueError):
            list_utils.get_index_of_first_crossing(values=values, target=1.0)
        ...
        values = [0.1, 0.5, 0.9]
        index = list_utils.get_index_of_first_crossing(values=values, target=0.9)
        self.assertEqual(index, 2)
        ## no crossing found (returns none)
        values = [0.1, 0.2, 0.3]
        result = list_utils.get_index_of_first_crossing(values=values, target=0.25, direction="falling")
    ...
    def test_sample_returns_expected_length(self):
        ...
        def _test(_input_length, _sampled_length):
            ...
            out_elems = list_utils.sample_list(elems=elems, max_elems=_sampled_length)
    ...
    def test_sample_expected_elems_divisible_cases(self):
        ...
        ## 20 % 4 == 0
        elems = list(range(21))
        out = list_utils.sample_list(elems=elems, max_elems=5)
        self.assertEqual(out, [0, 5, 10, 15, 20])
        ## 12 % 4 == 0
        elems = list(range(13))
        out = list_utils.sample_list(elems=elems, max_elems=5)
        self.assertEqual(out, [0, 3, 6, 9, 12])
    ...
    def test_sample_expected_elems_nondivisible_cases(self):
        ...
        elems = list(range(20))
        out = list_utils.sample_list(elems=elems, max_elems=5)
        self.assertEqual(out, [0, 4, 8, 12, 16])
    ...
    def test_first_element_and_constant_stringide(self):
        elems = list(range(37))
        max_elems = 7
        out = list_utils.sample_list(elems=elems, max_elems=max_elems)
    ...
    def test_single_element_requested(self):
        elems = [1, 2, 3]
        out = list_utils.sample_list(elems=elems, max_elems=1)
        self.assertEqual(out, [1])

    def test_invalid_inputs(self):
        with self.assertRaises(ValueError):
            list_utils.sample_list(elems=[], max_elems=3)
        with self.assertRaises(ValueError):
            list_utils.sample_list(elems=[1, 2, 3], max_elems=-1)
