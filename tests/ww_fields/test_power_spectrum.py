...
def main():
    ...
    ax.plot(k_modes, power_spectrum, color="blue", ls="-", lw=2.5, marker="o", ms=5)
    ax.plot(_k_modes, _power_spectrum, color="red", ls="-", lw=1.0, marker="o", ms=2)
    ax.set_xscale("log")
    ax.set_yscale("log")
    directory = io_manager.get_caller_directory()
    file_name = "demo_power_spectrum.png"
