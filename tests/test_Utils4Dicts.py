...
class Test_Utils4Dicts(unittest.TestCase):
  ...
  def test_mergeSimpleDicts(self):
    ...
    dict_1 = {"a": 1, "b": 2}
    dict_2 = {"b": 3, "c": 4}
    dict_merged = Utils4Dicts.mergeDicts(dict_1, dict_2)
  ...
  def test_filterDict2ExcludeKeys(self):
    ...
    dict_in = {"a": 1, "b": 2, "c": 3}
    filtered = Utils4Dicts.filterDict2ExcludeKeys(dict_in, ["b"])
    ...
    filtered_empty = Utils4Dicts.filterDict2ExcludeKeys({}, ["b"])
    self.assertEqual(filtered_empty, {})
    ## edge case 2: no keys to exclude
    filtered_none_exclude = Utils4Dicts.filterDict2ExcludeKeys(dict_in, [])
    self.assertEqual(filtered_none_exclude, dict_in)
    ## edge case 3: all keys are excluded
    filtered_all_exclude = Utils4Dicts.filterDict2ExcludeKeys(dict_in, ["a", "b", "c"])
    self.assertEqual(filtered_all_exclude, {})
    ## edge case 4: nested key is excluded
    dict_with_nested = {"a": 1, "b": {"c": 2}}
    filtered_nested = Utils4Dicts.filterDict2ExcludeKeys(dict_with_nested, ["b"])
  ...
  def test_checkIfDictsAreDifferent(self):
    ...
    dict1 = {"a": 1, "b": 2}
    dict2 = {"a": 1, "b": 2}
    self.assertFalse(Utils4Dicts.checkIfDictsAreDifferent(dict1, dict2))
    ## typical case 2: different dictionaries
    dict3 = {"a": 1, "b": 3}
    dict4 = {"a": 1}
    self.assertTrue(Utils4Dicts.checkIfDictsAreDifferent(dict1, dict3))
    self.assertTrue(Utils4Dicts.checkIfDictsAreDifferent(dict1, dict4))
    ## edge case 1: comparing two empty dictionaries
    dict_empty1 = {}
    dict_empty2 = {}
    self.assertFalse(Utils4Dicts.checkIfDictsAreDifferent(dict_empty1, dict_empty2))
    ## edge case 2: comparing an empty dictionary with a non-empty one
    dict_empty_non_empty = {}
    dict_non_empty = {"a": 1}
    self.assertTrue(Utils4Dicts.checkIfDictsAreDifferent(dict_empty_non_empty, dict_non_empty))
    ## edge case 3: comparing dictionaries with lists in the same key
    dict_with_list1 = {"a": [1, 2], "b": 3}
    dict_with_list2 = {"a": [1, 2], "b": 4}
    self.assertTrue(Utils4Dicts.checkIfDictsAreDifferent(dict_with_list1, dict_with_list2))
    ## edge case 4: comparing nested dictionaries
    dict_with_nested1 = {"a": 1, "b": {"c": 2}}
    dict_with_nested2 = {"a": 1, "b": {"c": 3}}
    self.assertTrue(Utils4Dicts.checkIfDictsAreDifferent(dict_with_nested1, dict_with_nested2))
    ## edge case 5: comparing identical nested dictionaries
    dict_with_nested_same = {"a": 1, "b": {"c": 2}}
    self.assertFalse(Utils4Dicts.checkIfDictsAreDifferent(dict_with_nested1, dict_with_nested_same))
