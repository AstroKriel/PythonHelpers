...
def time_function(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            raise RuntimeError(
                f"Error occurred in {func.__name__}() while measuring the elapsed time.",
            ) from error
...
def warn_if_result_is_unused(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        call_line = inspect.getsource(calling_frame).split("\n")[calling_frame.f_lineno -
                                                                 calling_frame.f_code.co_firstlineno]
        if ("=" not in call_line) and ("return" not in call_line) and (result is not None):
            warnings.warn(
                f"Return value of {func.__name__}() is not being used",
                UserWarning,
                stacklevel=2,
            )
        return result
