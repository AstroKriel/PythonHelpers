...
class MessageType(Enum):
  GENERAL = _MessageStyle("GENERAL", _Symbols.ARROW.value,         _Colours.WHITE.value)
  ...
  LIST    = _MessageStyle("LIST",    _Symbols.TAG.value,           _Colours.BLUE.value)
  ...
  def style(self) -> _MessageStyle:
    ...
    if self.message_type.requires_outcome():
      ...
      if self.action_outcome is not None:
        raise ValueError("'action_outcome' should not be set when Message.message_type != MessageType.ACTION.")
...
def render_block(
  ...
  row_prefix = _Symbols.DASH.value
