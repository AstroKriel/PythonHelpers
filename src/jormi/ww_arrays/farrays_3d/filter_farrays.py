...
from jormi.ww_arrays._filter_spectra import BandpassFilteredFFT as BandpassFilteredFFT
from jormi.ww_arrays._filter_spectra import FilterWindow as FilterWindow
...
def compute_bandpass_filtered_fft_farray(
    *,
    ...
    returning the filtered field alongside the Fourier-space state (before and after
    masking) that produced it.
    """
    return _filter_spectra.compute_bandpass_filtered_fft(
        farray=farray_3d,
...
def compute_bandpass_filtered_fft_sarray(
    *,
    ...
    """Band-pass filter a 3D scalar array in Fourier space, also returning its FFT state."""
    farray_types.ensure_3d_sarray(
        sarray_3d=sarray_3d,
        param_name="<sarray_3d>",
    )
    return compute_bandpass_filtered_fft_farray(
        farray_3d=sarray_3d,
...
def compute_bandpass_filtered_fft_varray(
    *,
    ...
    """Band-pass filter a 3D vector array in Fourier space, also returning its FFT state."""
...
def compute_bandpass_filtered_fft_r2tarray(
    *,
    ...
    """Band-pass filter a 3D rank-2 tensor array in Fourier space, also returning its FFT state."""
