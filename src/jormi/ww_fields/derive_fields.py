...
def compute_magnetic_curvature_terms(
    ...
    ## t_i t_j du_j/dx_i
    sfield_stretching = numpy.einsum(
        "ixyz,jxyz,jixyz->xyz",
        vbasis_tangent,
        vbasis_tangent,
        r2tensor_grad_u,
...
def compute_lorentz_force_terms(
    ...
    vbasis_tangent, vbasis_normal, _, sfield_kappa = decompose_fields.compute_tnb_terms(
        vfield_b,
        box_length,
        grad_order,
    )
    sfield_sq_magn_b = numpy.square(field_operators.compute_vfield_magnitude(vfield_b))
    vfield_tot_grad_pressure = 0.5 * field_operators.compute_sfield_gradient(
        sfield_sq_magn_b,
        box_length,
        grad_order,
    )
    vfield_align_grad_pressure = numpy.einsum(
        "ixyz,jxyz,jxyz->ixyz",
        vbasis_tangent,
        vbasis_tangent,
        vfield_tot_grad_pressure,
    )
...
def compute_dissipation_function(vfield_u: numpy.ndarray) -> numpy.ndarray:
    ...
    vfield_df = numpy.array(
        [
            numpy.sum(
                field_operators.compute_vfield_gradient(r2tensor_srt[:, 0, :, :, :])[0],
                axis=0,
            ),
            numpy.sum(
                field_operators.compute_vfield_gradient(r2tensor_srt[:, 1, :, :, :])[1],
                axis=0,
            ),
            numpy.sum(
                field_operators.compute_vfield_gradient(r2tensor_srt[:, 2, :, :, :])[2],
                axis=0,
            ),
        ],
    )
