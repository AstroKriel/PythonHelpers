...
def estimate_pdf(
  ...
  if numpy.std(values) < delta_threshold:
    ...
    epsilon = 1e-4 * (1.0 if mean_value == 0 else numpy.abs(mean_value))
  ...
  if numpy.any(bin_widths <= 0): raise ValueError("Error: All bin widths must be positive.")
  ...
  if weights is not None:
    if len(weights) != len(values): raise ValueError(f"Error: The length of weights ({len(weights)}) must match the length of values ({len(values)}).")
    numpy.add.at(bin_counts, bin_indices, weights)
  else: numpy.add.at(bin_counts, bin_indices, 1)
  total_counts = numpy.sum(bin_counts)
  if total_counts <= 0: raise ValueError("Error: None of the provided `values` fell into any bins. Check input data or binning:", values)
...
def create_uniformly_spaced_bin_centers(
  ...
  return numpy.linspace(
    start = data_p16 - (1 + bin_range_percent) * (data_p50 - data_p16),
    stop  = data_p84 + (1 + bin_range_percent) * (data_p84 - data_p50),
    num   = int(num_bins)
  )
