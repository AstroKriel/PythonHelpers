...
## FUNCTIONS
## ###############################################################
def generate_gaussian_random_sfield(size: int, correlation_length: float, num_dims:int =3):
...
def generate_powerlaw_sfield(
  ...
  - If `alpha_perp` is None, the field will be isotropic, with k^(-alpha_para).
  - If `alpha_perp` is provided, the field will be anisotropic, with k_para^(-alpha_para) * k_perp^(-alpha_perp).
  """
  ...
  if alpha_perp is None:
    ...
    amplitude = grid_k_magn_perp**(-alpha_perp / 2.0) * grid_k_magn_para**(-alpha_para / 2.0)
