def submitJob(directory, job_name, bool_check_job_status=False):
  if bool_check_job_status and WWFnF.checkIfJobIsInQueue(directory, job_name):
...
def checkIfJobIsInQueue(directory, job_filename):
  if not checkIfFileExists(directory, job_filename):
  ...
  try:
    ...
    return False
  job_tagname = None
  with open(createFilepathString(directory, job_filename), "r") as fp:
