...
from Loki.TheUsefulModule import WWFnF, WWFuncs, WWLists
from Loki.TheFlashModule import FileNames
...
def loadSpectrum(filepath_file, spectrum_name, spectrum_component="total"):
  with open(filepath_file, "r") as fp:
    ...
    elif "mag" in spectrum_name.lower(): data_power = data_power / (8 * np.pi)
    elif "cur" in spectrum_name.lower(): data_power = data_power / (4 * np.pi)
