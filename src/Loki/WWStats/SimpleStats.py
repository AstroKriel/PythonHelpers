...
def computeNorm(array1, array2, p=2, bool_normalise=False):
  ...
  if np.all(array1 == array2): return 0
  array_diff = np.abs(array1 - array2)
  if p == np.inf: return np.max(array_diff)
  elif p == 1:
    ## L1 norm: sum of absolute differences
    result = np.sum(array_diff)
    if bool_normalise: result /= len(array1)
  elif p == 0:
    ## L0 pseudo-norm: count of non-zero elements
    result = np.count_nonzero(array_diff)
  elif p > 0:
    ## general case for p > 0
    ## note numerical stability improvement: scale by maximum value
    max_diff = np.max(array_diff)
    if max_diff > 0:
        scaled_diff = array_diff / max_diff
        result = max_diff * np.power(np.sum(np.power(scaled_diff, p)), 1/p)
        if bool_normalise: result /= np.power(len(array1), 1/p)
    else: result = 0
