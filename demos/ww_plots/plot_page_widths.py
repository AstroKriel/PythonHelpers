...
def add_shape_legend(
    *,
    ...
    annotate_panel.add_custom_legend(
        ...
        anchor_point_fraction=(0.975, 0.2),
...
def add_trend_legend(
    *,
    ...
    annotate_panel.add_custom_legend(
        ...
        labels=[rf"$m = {series.slope:.2f}, \; \sigma = {series.sigma:.2f}$" for series in all_series],
        colors=[series.color for series in all_series],
        anchor_point_fraction=(0.975, 0.025),
        anchor_at_corner=box_positions.Positions.Corner.BottomRight,
    )
...
def add_residual_inset(
    *,
    ...
    inset_panel = manage_figure.add_inset_panel(
        panel=panel,
        bounds_fraction=(0.035, 0.65, 0.35, 0.3),
        x_label=r"$x$",
        y_label=r"$y - m\,x$",
...
def main() -> None:
    ...
    for label, figure_layout, is_full_page in (
        ...
        series_to_draw = (
            all_series if is_full_page else [dataclasses.replace(all_series[0], color=SINGLE_SERIES_COLOR)]
        )
