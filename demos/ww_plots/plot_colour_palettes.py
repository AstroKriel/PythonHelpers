...
def main() -> None:
    ...
    figure, panel_grid = manage_figure.create_figure(
        ...
        panel_row_gap_pt=14.0,
        panel_col_gap_pt=16.0,
    )
    col_sides = (
    ...
    for row_index, (kind, (named_palette, custom_palette)) in enumerate(palettes.items()):
        for col_index, palette in enumerate((named_palette, custom_palette)):
            route = "from_name" if col_index == 0 else "from_colors"
            draw_palette(
                panel=panel_grid[row_index, col_index],
                array_2d=array_2d,
                palette=palette,
                label=rf"{kind}: \texttt{{{route}}}",
                colorbar_side=col_sides[col_index],
            )
