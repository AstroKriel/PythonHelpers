...
class TestGradientWLSConvergence:
    ...
    def __init__(
        ...
        self.full_half_width = 4.0
        self.scored_half_width = 2.0
        self.k_neighbors = 20
        self.num_points_to_test = [2_000, 4_000, 8_000, 16_000, 32_000, 64_000]
        self.ax: manage_plots.PlotAxis = None  # pyright: ignore[reportAttributeAccessIssue]

    def run(
        self,
    ) -> None:
        fig, self.ax = manage_plots.create_figure(fig_scale=1.25)
        typical_spacings, rms_errors = self._measure_convergence()
        fitted_slope = self._plot_convergence(typical_spacings, rms_errors)
        file_name = "gradient_wls_convergence.png"
        file_path = Path(__file__).parent / file_name
        manage_plots.save_figure(
            fig=fig,
            fig_path=file_path,
        )
        ## error should decrease with resolution (positive order), and not by an
        ## implausibly large exponent that would suggest an overfit rather than real convergence
        converged = bool(0.3 < fitted_slope.value < 3.0)
        assert converged, (
    ...
    def _plot_convergence(
        ...
        self.ax.set_xlabel(r"$1/\tilde{\Delta x} \sim (N_{\rm points} / V)^{1/3}$")
        self.ax.set_ylabel(r"$e \equiv {\rm RMS}\,|\nabla f - \nabla f^*|$")
        self.ax.legend(loc=box_positions.MPLPositions.Anchor.Corner.TopRight)
        return fitted_slope
