...
def writeFlashParamFile(
    filepath_ref, filepath_to, dict_sim_inputs, max_hours,
    str_restart = ".false.",
    str_chk_num = "0",
    str_plt_num = "0"
  ):
  ...
  _addParamAssign(
    param_name = "cfl",
    param_value = dict_sim_inputs["cfl"]
  )
  _addParamAssign(
  ...
  _addParamAssign(
    ...
    param_value = dict_sim_inputs["init_rms_b"],
  )
  _addParamAssign(
    param_name  = "iProcs",
  ...
  _addParamAssign(
    ...
    param_value = "{:.6f}".format(dict_sim_inputs["t_turb"] / dict_sim_inputs["outputs_per_t_turb"]),
    comment     = "1/{:d} t_turb".format(dict_sim_inputs["outputs_per_t_turb"])
  )
  _addParamAssign(
    param_name  = "restart",
...
## ###############################################################
class JobRunSim():
  def __init__(self, directory_sim, dict_sim_inputs, max_hours=None):
  ...
  def _defineJobParams(self):
    self.run_index = self.dict_sim_inputs["run_index"]
    ...
    self.job_tagname = FlashData.getJobTag(self.dict_sim_inputs, "sim")
    ...
    k_turb = self.dict_sim_inputs["k_turb"]
    if k_turb < 2: raise Exception(f"Error: k_turb cannot be < 2. You have requested k_turb = {k_turb}")
    if WWFnF.checkFileExists(self.directory_sim, FileNames.FILENAME_DRIVING_HISTORY):
      des_amplitude = FlashData.readDrivingAmplitude(self.directory_sim)
    else: des_amplitude = 0.1
    self.dict_driving_params = {
      "des_k_driv"            : k_turb,
      "des_k_min"             : k_turb - 1,
      "des_k_max"             : k_turb + 1,
      "des_velocity"          : self.dict_sim_inputs["desired_Mach"],
  ...
  def prepForRestart(self):
    ...
    last_chk_filename = FlashData.getLastChkFilename(self.directory_sim)
  ...
  def prepFromReference(self, filepath_ref_sim):
    ...
    ref_sim_input = FlashData.readSimInputs(filepath_ref_sim, bool_verbose=False)
    self.dict_sim_inputs["cfl"]                = ref_sim_input["cfl"]
    self.dict_sim_inputs["init_rms_b"]         = ref_sim_input["init_rms_b"]
    self.dict_sim_inputs["max_num_t_turb"]     = ref_sim_input["max_num_t_turb"]
    self.dict_sim_inputs["bool_driving_tuned"] = ref_sim_input["bool_driving_tuned"]
